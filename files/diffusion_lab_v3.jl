...
md"""
Diffusion de  $(@bind type_diff PlutoUI.Select(["Particules","Molécules"]))  
"""
...
begin
	if type_diff=="Particules"
		md"""
		Il faut définir le rayon effectif ( _r_ =  $(@bind val_r PlutoUI.Select([0.001, 0.01, 0.1],default=0.001)) micromètre(s) ). 
		"""

	else
		set_particles = Dict("Albumine" => 6.1e-7,"Sucrose"=> 4.7e-6, "Li+"=> 9.4e-5,"Na+"=> 1.2e-5)
		md"""Faire votre choix de molécule: $(@bind tosim PlutoUI.Select(collect(keys(set_particles))))"""
	end
end
...
@parameters t x Dcoeff
@variables u(..)
Dt = Differential(t)
Dx = Differential(x)
Dxx = Differential(x)^2

## diffusion coefficient at 20Celcius
kb = 1.38e-23 # J/Kelvin
T = 293.0 # Kelvin -> 20 Celcius
eta = 1.005e-3 # kg/m/s -- dynamic viscosity 

if type_diff=="Particules"
	r = val_r
	Dcoeff = kb*T/(6*pi*r*eta)*1e10 # cm2 / s
else
	Dcoeff = set_particles[tosim]
end

# 1D PDE and boundary conditions
eq  = Dt(u(t, x)) ~ Dcoeff*Dxx(u(t, x))
bcs = [u(0, x) ~ exp(-(x)^2/0.0001),
        Dx(u(t, -0.05)) ~ 0.0,
        Dx(u(t, 0.05)) ~ 0.0]
...
begin
	...
	Plots.plot!(discrete_x, solu[val_t, :], ylim = (0,1.1), xlim = (xmin, xmax), title="Diffusion", label=str_time , linewidth=3,
...
begin
	...
	Plots.scatter!(discrete_t[val_t:val_t], solu[val_t, 101:101], label="", mc=:green, ms=6, ma=0.5)
end
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "bc7cbf21158729e9922b890f13041b2b7302ee64"
